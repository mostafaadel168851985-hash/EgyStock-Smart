...
def show_report(code, p, h, l, v):
    ...
    s1_disp = f"{s1:.2f}" if s1 else "غير متاح"
    s2_disp = f"{s2:.2f}" if s2 else "غير متاح"
    ...
    أقرب دعم عند <b>{s1_disp}</b> يليه دعم أقوى عند <b>{s2_disp}</b>.<br>
...
📌 التوصية: {rec}
📝 ملحوظة للمحبوس: أقرب دعم عند {s1_disp}، يليه دعم أقوى عند {s2_disp}.
"""
...
def scanner():
    ...
    for s in WATCHLIST:
        ...
        result = f"{s} | السعر {p:.2f} | دعم {s1_disp}/{s2_disp} | مقاومة {r1:.2f}/{r2:.2f} | RSI {rsi:.1f} | سيولة {liq} | {rev_txt} | {conf_txt} | 🎯 المضارب: دخول {ai['trader']['entry']}, وقف خسارة {ai['trader']['sl']} | 🔁 السوينج: دخول {ai['swing']['entry']}, وقف خسارة {ai['swing']['sl']} | 🏦 المستثمر: دخول {ai['invest']['entry']}, وقف خسارة {ai['invest']['sl']}"
