...
st.set_page_config(page_title="EGX Ultimate Sniper v19", layout="centered")
...
def get_auto_data(ticker):
    try:
        ...
        return {"p": p, "rsi": rsi, "vol": vol, "s_t": "صاعد 🟢" if p > ma20 else "هابط 🔴", "m_t": "صاعد 🟢" if p > ma50 else "هابط 🔴", "l_t": "صاعد 🟢" if p > ma100 else "هابط 🔴"}
    except: return None
...
# --- 2. لوحة الإدخال اليدوي ---
st.markdown("<hr style='border-color:#333;'>", unsafe_allow_html=True)
st.markdown("<h3 style='color:white; text-align:center;'>🛠️ لوحة القناص اليدوية</h3>", unsafe_allow_html=True)

col1, col2, col3 = st.columns(3)
with col1: m_p = st.number_input("💵 السعر الآن:", format="%.3f", key="v1")
with col2: m_h = st.number_input("🔝 أعلى سعر:", format="%.3f", key="v2")
with col3: m_l = st.number_input("📉 أقل سعر:", format="%.3f", key="v3")

col4, col5, col6 = st.columns(3)
with col4: m_cl = st.number_input("↩️ إغلاق أمس:", format="%.3f", key="v4")
with col5: m_mh = st.number_input("🗓️ أعلى شهر:", format="%.3f", key="v5")
with col6: m_v = st.number_input("💧 قيمة التداول (M):", format="%.2f", key="v6")
...
if m_p > 0 and m_h > 0:
    ...
    <div class="report-card" style="border-right: 8px solid #3498db;">
        ...
        <a href="https://wa.me/?text=تحليل يدوي: {m_p:.3f}" class="wa-button">🚀 مشاركة التقرير اليدوي</a>
    </div>
    """, unsafe_allow_html=True)
